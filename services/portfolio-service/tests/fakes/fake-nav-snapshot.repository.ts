...
export class FakeNavSnapshotRepository extends NavSnapshotRepository {
  ...
  insertIgnoreOnConflict(input: {
    userPortfolioId: string;
    forDate: string;
    navValue: string;
  }): any {
    const key = `${input.userPortfolioId}|${input.forDate}`;
  ...
  async latestForUserPortfolio(userPortfolioId: string): Promise<NavSnapshot | null> {
    const all = [...this.byKey.values()].filter((s) => s.userPortfolioId === userPortfolioId);
